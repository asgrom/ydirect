        ...
        for query in words:
            ...
            for page in tqdm(range(int(pages))):
                ...
                for item in g.doc.select('//div[@class="banner-selection"]'):
                    ...
                    ad_title = re.sub(r'"', '', item.select(
                        './div[@class="ad"]/div[@class="ad-link"]').text())
                    ad_body = re.sub(r'"', '', item.select(
                        './div[@class="ad"]/div')[-1].text())

                    # пробуем получить название компании, телефон и почту
                    try:
                        # получаем ссылку на карточку объявления
                        ad_url = item.select(
                            './div[@class="ad"]/span/a[@class="vcard"]').attr('href')
                        g1 = Grab()
                        g1.go(ad_url)

                        try:
                            ad_h1 = re.sub(
                                r'"', '', g1.doc.select('//h1').text())
                        except:
                            ad_h1 = ''
                    ...
                    try:
                        ad_domain = item.select(
                            './div[@class="ad"]/span/span[@class="domain"]').text()
                    except:
                        ad_domain = ''
                    ...
                    if ad_phone in uniq_phones:
                        ...
                        f.write('"'+ad_h1+'";"'+ad_phone+'";"'+ad_email+'";"' +
                                ad_title+'";"'+ad_body+'";"'+ad_domain+'"'+'\n')
